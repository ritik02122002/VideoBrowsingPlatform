            ...
            <div className='flex p-1 m-1 my-1 justify-between flex-wrap *:mx-2 *:mb-3'>
                ...
                <div className='relative group'><h4 className=' h-max cursor-pointer bg-gray-100 hover:bg-gray-200 rounded-md border-gray-300  py-1'>
                    ...
                    {isShareContainerVisible && <div className='fixed bg-white shadow-inner shadow-gray-400 z-10 rounded-md top-20 sm:top-52 left-[3vw] sm:left-1/4' >
                        ...
                        <button className='text-white bg-black text-sm sm:text-base rounded-full p-2 mx-1 my-2 hover:text-black hover:bg-white hover:border-2 hover:border-black sm:hidden ' onClick={() => { copyContent(window.location.href) }}>Copy Link</button>
                            <a target='_blank' href={WHATSAPP_SEND_MSG_API_URL + 'Watch this amazing video : ' + vData?.items[0]?.snippet?.title + "%0a Link: " + window.location.href}><div className='w-max py-2 px-3 mx-1 my-2 hover:bg-gray-100 rounded-md'><i className="fa fa-whatsapp  fa-3x text-green-600"></i><h2 className='text-sm'>WhatsApp</h2></div></a>
                            <a target='_blank' href={TELEGRAM_SEND_MSG_API_URL + window.location.href + "&text=Watch this amazing video : " + vData?.items[0]?.snippet?.title}><div className='w-max py-2 px-2 mx-1 my-2 hover:bg-gray-100 rounded-md'><i className="fa fa-telegram fa-3x text-sky-500 "></i><h2 className='text-sm'>Telegram</h2></div></a>
                            <a href={"mailto:?subject=Watch this amazing video : " + encodeURIComponent(vData?.items[0]?.snippet?.title) + "&body=Link : " + encodeURIComponent(window.location.href)}><div className='w-max py-2 px-2 mx-1 my-2 hover:bg-gray-100 rounded-md'><i className="fa fa-envelope fa-3x text-gray-500"></i><h2 className='text-sm'>Email</h2></div></a>
                        </div>
                        <div className='py-1 px-2 my-1 mx-1 border-gray-600 border-2 whitespace-nowrap font-semibold text-xs sm:text-sm rounded-md sm:block hidden'>{window.location.href} <button className='text-white bg-black font-semibold text-sm sm:text-base rounded-md p-2 mx-3 my-2 hover:text-black hover:bg-white hover:border-2 hover:border-black' onClick={() => { copyContent(window.location.href) }}>Copy</button></div>
                    </div>}
                </div>
