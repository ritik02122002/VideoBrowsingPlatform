  ...
  return (
    <Link to={"/watch?v=" + id}>
      <div className='p-1 m-2 hover:shadow-md hover:shadow-gray-600 hover:bg-gray-50 rounded-md relative'>
        ...
        <img src={thumbnail?.maxres ? thumbnail?.maxres?.url : (thumbnail?.standard ? thumbnail?.standard?.url : (thumbnail?.high ? thumbnail?.high?.url : (thumbnail?.medium ? thumbnail?.medium?.url : thumbnail?.default?.url)))} alt='video thumbnail' className='rounded-lg h-40 w-72 items-stretch' />
        ...
        <h2 className='text-md font-semibold overflow-hidden w-72 p-1 line-clamp-2'>{title}</h2>
