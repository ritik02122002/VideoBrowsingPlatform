  ...
  return (
    <Link to={includeLink ? id : ""}>
      <div className='p-1 m-2 rounded-md relative group hover:px-3 hover:py-3 w-max h-max'>
        ...
        <img src={thumbnail?.maxres ? thumbnail?.maxres?.url : (thumbnail?.standard ? thumbnail?.standard?.url : (thumbnail?.high ? thumbnail?.high?.url : (thumbnail?.medium ? thumbnail?.medium?.url : thumbnail?.default?.url)))} alt="playlist thumbnail" className='rounded-lg h-40 w-72 items-stretch group-hover:h-36 group-hover:w-64' />
        ...
        <h2 className='text-md font-semibold overflow-hidden w-72 p-1 line-clamp-2 group-hover:w-64 group-hover:text-sm'>{title}</h2>
